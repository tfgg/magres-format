...
class ConvertGUI(object):
    ...
    def __init__(self, master, tot_height):
        ...
        self.castep_file_entry = Entry(self.castep_file_frame, background="white", width="15",
                                       textvariable=self.castep_file_name)
        ...
        self.castep_file_button = Button(self.castep_file_frame, text="Load CASTEP file (optional)",
                                         command=self.castep_file_load)
        ...
        self.out_file_entry = Entry(self.out_file_frame, background="white", width="15",
                                    textvariable=self.out_file_name)
...
if __name__ == "__main__":
    ...
    root.geometry(
        "{:d}x{:d}+{:d}+{:d}".format(int(scr_W * scr_prc), int(scr_H * scr_prc), int(scr_W * (1.0 - scr_prc) / 2.0),
                                     int(scr_H * (1.0 - scr_prc) / 2.0)))
